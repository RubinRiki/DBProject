...
1. ודאו כי PostgreSQL פועל והמסד כולל את כל הטבלאות מהשלבים הקודמים.
2. הפעילו את `json-server` (או שרת צד אחר) שמאזין לבקשות API מהקליינט.
3. פתחו את הקובץ `index.html` בדפדפן.
4. השתמשו בתפריט העליון לניווט בין המסכים: עובדים, מוצרים, ייצור ושאילתות.

---

### 🛠️ הסבר על דרך העבודה והכלים
האפליקציה נבנתה באמצעות HTML, CSS ו־JavaScript.  
העיצוב נבחר בקפידה בהשראת סגנון יקבים צרפתיים – עם צבעים כהים, גווני יין, טיפוגרפיה מסוגננת ופונטים מסוג `Lora`.  
- כל פעולה (הוספה, עדכון, מחיקה) מתבצעת מתוך טפסים במודל קופץ (modal).  
- הודעות אישור מוצגות בפעולות רגישות כמו מחיקה.  
- גרפים מוצגים באמצעות Chart.js.  
- הנתונים נמשכים בעזרת REST API המחובר ל־PostgreSQL דרך `json-server`.

המערכת כוללת 5 מסכים:
- **Home:** גרפים וסקירה כללית
- **Employees:** ניהול עובדים
- **Final Products:** ניהול מוצרים סופיים
- **Production:** תהליכי ייצור
- **Queries:** שאילתות ופרוצדורות שנכתבו בשלבים הקודמים
...
- **ניהול עובדים עם טופס הוספה/עדכון**
  ![](images/employees.png)

- **מוצרים סופיים**
  ![](images/finalproducts.png)
